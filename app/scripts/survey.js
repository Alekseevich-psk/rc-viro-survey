(function () {
    ...
    const form = parent.querySelector(".survey__form");
    const result = parent.querySelector(".survey__result");

    const options = parent.querySelectorAll(".survey__form-input");
    const surveyProgress = parent.querySelector(".survey__progress");
    const btn = parent.querySelector(".survey__btn");
    const surveyFormTitle = parent.querySelector(".survey__form-title span");
    const surveyFormDesc = parent.querySelector(".survey__form-desc");
    const surveyResultCount = parent.querySelector(".survey__result-count span");
    const surveyResultCountText = parent.querySelector(".survey__result-count-text");
    const surveyResultDesc = parent.querySelector(".survey__result-desc");
    ...
    const resStore = [
        "Поздравляем, Вы в ресурсе! У Вас нет симптомов астении. Однако для мониторинга своего состояния и своевременного выявления симптомов хронической усталости (астении), время от времени проходите данный тест. А также следите за выпусками «Долго, счастливо, осознано» и Вы узнаете как всегда оставаться в тонусе и все успевать!",

        "У Вас наблюдается слабо выраженные симптомы астении (хронической усталости). Устраните те виды деятельности, которые вызывают сильное эмоциональное напряжение, и старайтесь не перегружать свой день слишком большим количеством дел. А также, следите за выпусками «Долго, счастливо, осознано» и Вы узнаете, как победить симптомы астении и вернуть работоспособность!",

        "У Вас умеренно выраженная астения (хроническая усталость). Возьмите отпуск. Найдите баланс между работой и отдыхом. Если не будет улучшения самочувствия, обратитесь к специалистам. Для мониторинга астении (хронической усталости), время от времени проходите данный тест. А также, следите за выпусками «Долго, счастливо, осознано» и Вы узнаете, как победить симптомы астении и вернуть работоспособность!",

        "У Вас значительно выраженные симптомы астении (хронической усталости). Незамедлительно обратитесь к своему лечащему врачу. Для мониторинга астении (хронической усталости), время от времени проходите данный тест. А также, следите за выпусками «Долго, счастливо, осознано» и Вы узнаете, как победить симптомы астении и вернуть работоспособность!",
    ];

    createItemsProgress(store.length);
    const progressItems = parent.querySelectorAll(".survey__progress-item");
    initSurvey();
    ...
    function showResult() {
        const countRes = getResultCount();
        const textCount = ["Балл", "Балла", "Баллов"];
    ...
    function createItemsProgress(count) {
        for (let i = 0; i < count; i++) {
            const elem = `
            <div class="survey__progress-item"><span>${i + 1}</span></div>
            `;
